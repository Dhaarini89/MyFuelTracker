...
class RefuelActivity : AppCompatActivity()
{
    ...
    var refuelRec :List<DatabaseRefuel>? = emptyList()
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int)
            {
                ...
                if (direction == ItemTouchHelper.LEFT) {
                    ...
                    GlobalScope.launch(Dispatchers.IO) {
                        ...
                        val Record = refuelViewModel.getRefuelRecord(deletedModel.id +1)
                        if (Record !=null) {
                            Record.prev_kms = deletedModel.prev_kms
                            Record.prev_quantity = deletedModel.prev_quantity
                            refuelViewModel.updateRefuelRecord(Record)
                          //  refuelViewModel.recalculateAverageMileagebyPrevRecord(Record)
                        }
                        Log.d("AvgPositionDelete",position.toString())
                    }
                    GlobalScope.launch(Dispatchers.Main) {
                        refuelsRecord = refuelViewModel.getRefuelRecords()
                        Log.d("AvgDeleted",refuelViewModel.getTotalRefuelRecords().toString())
                        refuelsRecord?.let { it ->
                    ...
                    val snackbar = Snackbar.make(
                        Mainbinding.refuelRecordsRecyclerView,
                    ...
                    snackbar.setAction("UNDO") {
                        ...
                        GlobalScope.launch(Dispatchers.IO) {
                            ...
                            refuelViewModel.updateMileage(deletedModel)
                            val Record = refuelViewModel.getRefuelRecord(deletedModel.id +1)
                            if (Record !=null) {
                                Record.prev_kms = deletedModel.prev_kms
                                Record.prev_quantity = deletedModel.prev_quantity
    ...
    fun populaddrefueldialog(id: Int)
    {
        ...
        window?.setBackgroundDrawable(ColorDrawable(ContextCompat.getColor(this@RefuelActivity, R.color.light_gray)))
        dialog.show()
        binding.addButton.setOnClickListener {
            Log.d("AvgFuel", "Step 1")
            val value = validationRefuelForm(binding)
            loadingRecyclerview()

            if (value == 1) {
                Log.d("AvgFuel", "Step 14")
                dialog.dismiss()
            }
    ...
    fun validationRefuelForm(in_binding: ActivityRefuelBinding) :Int
    {
        GlobalScope.launch(Dispatchers.Main) {
            ...
            prev_kms_db = withContext(Dispatchers.IO) {
                ...
                refuelViewModel.getPrevKms()
            }
            if (in_binding.etOdometer.text!!.isEmpty() || ((Integer.parseInt(in_binding.etOdometer.text!!.toString()) <= prev_kms_db!!) && updateFlag == 0)) {
                flag = 0
                Log.d("AvgFuel", "Step 4" + "validation")
                in_binding.tilOdometer.setError("Please enter valid km in numbers,more than " + prev_kms_db.toString() + "Kms")

            } else if (in_binding.etTotalcost.text!!.isEmpty()) {
                flag = 0
                in_binding.tilTotalcost.setError("Please enter total cost of refueled")

            } else if (in_binding.etGasL.text!!.isEmpty()) {
                flag = 0
                in_binding.tilGasL.setError("Please enter total litres refueled")
            } else {
                Log.d("AvgFuel", "Step 5 after validation")
                prev_kms_db = 0
                in_binding.tilTotalcost.setError(null)
                in_binding.tilGasL.setError(null)
                in_binding.tilOdometer.setError(null)
                flag = 1
            }
            Log.d("AvgFuel", "Step 6")
            if (flag == 1) {
                Log.d("AvgFuel", "Step 7")
                val added=addRefuelToDb(in_binding)
            }
        }
        return flag
    }
        ...
        GlobalScope.launch(Dispatchers.Main) {
            ...
            if (updateFlag == 1) {
                ...
                withContext(Dispatchers.IO) {
                    ...
                    refuelRec = refuelViewModel.getRefuelRecords()
                    updateFlag = 0
                }
                ...
                withContext(Dispatchers.IO) {
                    ...
                    newRefuel.prev_kms = refuelViewModel.getPrevKms()
                    newRefuel.prev_quantity = refuelViewModel.getPrevQuantity()
                    refuelViewModel.addRefuel(newRefuel)
                   // refuelViewModel.calculateAverageMileagebyPreviousRecord()
